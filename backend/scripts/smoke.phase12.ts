...
interface ChatResponse {
  answer: string;
  sources?: Array<{ marker: number; chunkIndex: number; score: number }>;
  cached?: boolean;
  debug?: RagDebug;
}
...
function checkDebugShape(label: string, debug: RagDebug | undefined): void {
  ...
  const included = cands.filter((c) => c.included);
  const markers = included
    .map((c) => c.marker)
    .filter((m): m is number => typeof m === 'number');
  check(
    `${label}: included candidates carry unique numeric markers >= 1`,
    included.length > 0 &&
      markers.length === included.length &&
      markers.every((m) => Number.isInteger(m) && m >= 1) &&
      new Set(markers).size === markers.length,
    JSON.stringify(markers),
  );
  check(
    `${label}: non-included candidates carry no marker`,
    cands.every((c) => c.included || c.marker === undefined),
  );
  check(
...
async function main(): Promise<void> {
  ...
  // 6. Collection endpoint spot-check: debug works in collection chat mode.
  const colRes = await fetch(`${BASE}/collections`, {
    ...
    body: JSON.stringify({ name: 'smoke12-collection' }),
  });
  check('create collection → 2xx', colRes.ok, `status ${colRes.status}`);
  const col = (await colRes.json()) as CollectionResponse;
  const addRes = await fetch(`${BASE}/collections/${col.id}/documents`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...authHeaders(a.accessToken),
  ...
  check(
    'add document to collection → 2xx',
    addRes.ok,
    `status ${addRes.status}`,
  );
  const colChat = await fetch(`${BASE}/collections/${col.id}/chat`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...authHeaders(a.accessToken),
    ...
    body: JSON.stringify({
      question: 'When does the AURORA-7 project launch?',
      debug: true,
    }),
  });
  const colBody = (await colChat.json().catch(() => ({}))) as ChatResponse;
  check(
    'collection chat with debug → 2xx',
    colChat.ok,
    `status ${colChat.status}`,
  );
  checkDebugShape('collection debug', colBody.debug);
  check(
    'collection debug candidates carry documentId',
    (colBody.debug?.candidates ?? []).every(
      (c) => typeof c.documentId === 'string',
    ),
  );
  const delCol = await fetch(`${BASE}/collections/${col.id}`, {
    method: 'DELETE',
    headers: authHeaders(a.accessToken),
  });
