...
- **Fix**: Set `CORS_ORIGIN=http://localhost:8080` in backend `.env` (and `.env.example`) so the dev server origin is explicitly allowed.
...
1. **Backend**
   ...
   - **.env / .env.example**: Document `DATABASE_URL`, `REDIS_*`, `JWT_SECRET`, `JWT_EXPIRES_IN`, and set **CORS_ORIGIN=http://localhost:8080**.
...
| Backend | `.env` | Add `CORS_ORIGIN=http://localhost:8080`. |
...
- [ ] **Env**: `backend/.env` has `JWT_SECRET`, `DATABASE_URL`, `CORS_ORIGIN=http://localhost:8080`. No typo in `DATABASE_URL` (e.g. port 5432).
...
- [ ] **CORS**: From browser at `http://localhost:8080`, all API requests to `http://localhost:3000` succeed without “Failed to fetch” due to CORS (check Console and Network).
...
- **“Failed to fetch”**: Ensure backend is listening on the port in `VITE_API_URL` (default `http://localhost:3000`), and `CORS_ORIGIN` matches the frontend origin (e.g. `http://localhost:8080` for Vite dev).
